...
public class PlayerController : MonoBehaviour
{
    ...
    public CharacterController controller;              //Rigidbidy 를 사용하지 않는 충돌체 컨트롤 용
    ...
    public float dashCookdown = 5.0f;
    ...
    public void MoveLogic(bool isGrounded)
    {
        ...
        {
            if(velocity.y < 0)                          //y축 속도 값을 보고 판단한다.
            {
        ...
        Vector3 direction = new Vector3(horizontal, 0f, vertical).normalized;       //이동값을 0 ~ 1 사이로 만들어 준다.

        animator.SetFloat("moveSpeed", Mathf.Lerp(0, 1, currentSpeed / speed));

        if(direction.magnitude >= 0.1f)
        {
            float targetAngle = Mathf.Atan2(direction.x, direction.z) * Mathf.Rad2Deg + cam.eulerAngles.y;          //카메라가 보는 방향을 앞으로 되게 한다.
    ...
    {
        if (currentAction.fxObject != null)         //VFX가 Null 값이 아닐 때
        {
